...
@Composable
fun AlarmScreen(permissionRefresh: Int) {
    ...
    AppScreenColumn {
        ...
        InfoCard(
            "Alarmes repetidos são reagendados automaticamente. Alarmes de uma única vez são desativados depois de tocar."
        )
    }
    ...
    if (showEditor) {
        AlarmEditorDialog(
            ...
            onConfirm = { hour, minute, label, days ->
                saveAlarm(repository, scheduler, editing, hour, minute, label, days)
                alarms = repository.all()
                showEditor = false
                editing = null
            }
